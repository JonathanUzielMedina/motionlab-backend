...
interface SimulationState {
  ...
  isGoalOneCompleted: boolean;
  isGoalTwoCompleted: boolean;
  isGoalThreeCompleted: boolean;
  ...
  try {
    ...
    const calculateTotalProgress = (xPos: number) => {
      if (xPos <= flag1X) {
        ...
        const tramo2Percent =
          tramo2Progress * ((tramo2Pixels / totalCoursePixels) * 100);
        ...
        const tramo3Percent =
          tramo3Progress * ((tramo3Pixels / totalCoursePixels) * 100);
    ...
    const calculateAcceleration = (velocity: number, xPos: number) => {
      ...
      if (xPos >= rampStartX && xPos <= rampEndX) {
        ...
        const frictionForce =
          frictionCoefficient * totalMass * GRAVITY * Math.cos(rampAngle);
        hillResistance += frictionForce;
      }

      const availablePower = motorPower * HP_TO_WATTS;
      const motorForce =
        velocity > 0.001 ? availablePower / velocity : availablePower * 10;
      const netForce =
        massAdjustedAcceleration * velocityFactor * totalMass -
        hillResistance -
    ...
    for (let frame = 0; frame < MAX_FRAMES && !completed; frame++) {
      ...
      // Check if on ramp
      const isOnRamp =
        currentState.position.x >= rampStartX &&
      ...
      if (isOnRamp && Math.abs(newVelocity) < 0.05) {
        ...
        const progressPercent = Math.round(
          calculateTotalProgress(currentState.position.x)
        );
        currentState = {
          ...currentState,
          velocity: 0,
      ...
      const distanceIncrement =
        currentState.velocity * DELTA_TIME +
        0.5 * acceleration * Math.pow(DELTA_TIME, 2);
      const newDistanceTraveled =
        currentState.distanceTraveled + distanceIncrement;
      ...
      currentState = {
        ...
        isGoalOneCompleted:
          currentState.isGoalOneCompleted || isRampBaseReached,
        ...
        statusMessage:
          isRampTopReached && !currentState.isGoalTwoCompleted
